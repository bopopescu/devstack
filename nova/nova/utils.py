...
def execute(*cmd, **kwargs):
    ...
    if 'run_as_root' in kwargs and 'root_helper' in kwargs:
        kwargs['root_helper'] = _get_root_helper1()
...
def get_curr_frequency():
    """Get the currently operating maximum frequency
    """
    curr_freq = '3400000'
    try:
        curr_freq = execute('cat', '/sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq')

        if not curr_freq:
            return int(curr_freq[0])
    except Exception as ex:
        LOG.error(_LE("Couldn't get frequency : %(ex)s"), {'ex': ex})
    return int(curr_freq[0])
...
def get_max_frequency():
    ...
    max_freq = '3400000'
    try:
        max_freq = execute('cat', '/sys/devices/system/cpu/cpu0/cpufreq/cpuinfo_max_freq')

        if not max_freq:
            return int(max_freq[0])
    except Exception as ex:
        LOG.error(_LE("Couldn't get frequency : %(ex)s"), {'ex': ex})
    return int(max_freq[0])
...
def get_all_frequency():
    ...
    all_freq = ''
    try:
        all_freq = execute('cat', '/sys/devices/system/cpu/cpu0/cpufreq/scaling_available_frequencies')
...
def set_curr_frequency():
    """Set the currently operating maximum frequency
    """
    num_vcpu=8
    upper_limit_freq=3401000 #shd be passed as a parameter of this function
    lower_limit_freq=1600000
    set_freq=3400000
    for core in range(0,num_vcpu):
        try:
            execute('/usr/bin/cpufreq-set', '-c', core, '-u', upper_limit_freq, run_as_root=True,root_helper=False)
            execute('/usr/bin/cpufreq-set', '-c', core, '-d', lower_limit_freq, run_as_root=True,root_helper=False)
            execute('/usr/bin/cpufreq-set', '-c', core, '-u', set_freq, run_as_root=True,root_helper=False)
            execute('/usr/bin/cpufreq-set', '-c', core, '-d', set_freq, run_as_root=True,root_helper=False)
        except Exception as ex:
            LOG.error(_LE("Couldn't get frequency : %(ex)s"), {'ex': ex})
#    return int(curr_freq[0])
##############
